...
class App extends Component {
  ...
  getProjectsFilterLists = () => {
    const {activeTabId} = this.state
    const getProjectsLists = projectsList.filter(
      eachProjectDetails => eachProjectDetails.category === activeTabId,
    )

    return getProjectsLists
  }

  render() {
    const getProjectsFilterLists = this.getProjectsFilterLists()
    const {activeTabId} = this.state
        ...
        <ul className="tabs-container">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              activeTabBtnItem={tabDetails.tabId === activeTabId}
            />
          ))}
        </ul>

        <ul className="project-list-container">
          {getProjectsFilterLists.map(projectDetails => (
            <ProjectItem
